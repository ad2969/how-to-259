...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates, char* output_string)
{
  ...
  // reset the output string before doing anything
  ...
  for (i = 0; i < number_of_candidates; i++) {
      ...
      // check if entire sequence is not equal
      if (strcmp(sample_segment, candidate_segments[i]) != 0) continue;

      // if all is equal, flag and create output string
      has_perfect_match += 1;

      sprintf(int_buffer, "%d", i + 1);
      strcpy(outputline_buffer, "Candidate number ");
      strcat(outputline_buffer, int_buffer);
      strcat(outputline_buffer, " is a perfect match\n");

      // copy to output_string
...
int calculate_score(char* sample_segment, char* candidate_segment)
{
  ...
  for (iteration = 0; iteration <= candidate_length_in_codons - sample_length_in_codons; iteration++) {
      temp_score = 0;

      for (codon = 0; codon < sample_length_in_codons; codon++) {
          sample_offset = codon * CODON_LENGTH;
          candidate_offset = (iteration * CODON_LENGTH) + sample_offset;

          if (strncmp(sample_segment + sample_offset, candidate_segment + candidate_offset, CODON_LENGTH) == 0) {
              temp_score += 10;
          }
          else if (strcmp(
              codon_names[get_codon_index(sample_segment + sample_offset)],
              codon_names[get_codon_index(candidate_segment + candidate_offset)]
          ) == 0) {
              temp_score += 5;
          }
          else {
              for (nucleotide = 0; nucleotide < CODON_LENGTH; nucleotide++) {
                  ...
                  else if (is_base_pair(sample_segment[sample_offset], candidate_segment[candidate_offset])) {
                      temp_score += 1;
                  }
              }
          }
