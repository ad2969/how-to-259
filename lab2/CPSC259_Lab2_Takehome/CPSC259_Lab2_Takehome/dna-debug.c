...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates, char* output_string)
{
  ...
  for (i = 0; i < number_of_candidates; ++i) {
      ...
      if (sample_length != candidate_length) continue;

      if (strcmp(sample_segment, candidate_segments[i]) != 0) continue;
      
      sprintf(int_buffer, "%d", i + 1);
      strcpy(outputline_buffer, "Candidate number "); // reset previous buffer
      strcat(outputline_buffer, int_buffer);
      strcat(outputline_buffer, " is a perfect match\n");
...
int calculate_score(char* sample_segment, char* candidate_segment)
{
  ...
  int sample_offset = 0, candidate_offset = 0;

  // Insert your code here (replace this return statement with your own code)

  // iterate through each "shift", where the number of shifts = candidate_length_in_codons - sample_length_in_codons
  for (iterations = 0; iterations < candidate_length_in_codons - sample_length_in_codons;  ++iterations) {
      temp_score = 0;

      // don't have to worry about this, but add as a check anyways
      if (candidate_length_in_codons - iterations < sample_length_in_codons) continue;

      printf("\n\n# ITERATION NUMBER = %d\n", iterations);
      printf("Comparing STRINGS --> '%s' and '%s'\n\n", sample_segment, candidate_segment + (iterations * CODON_LENGTH));

      // iterate through each codon, where the max of codons is based on the sample
      for (int codon = 0; codon < sample_length_in_codons; ++codon) {
          sample_offset = codon * CODON_LENGTH;
          candidate_offset = sample_offset + (iterations * CODON_LENGTH);

          printf("\n## CHECKING DIFFERENT CODON COMBINATION with offsets = %d, %d\n", sample_offset, candidate_offset);

          printf("### Comparing CODONS --> '%.3s' and '%.3s'\n", sample_segment + sample_offset, candidate_segment + candidate_offset);

          // check if the codons are exactly the same (10 points)
          if (strncmp(sample_segment + sample_offset, candidate_segment + candidate_offset, CODON_LENGTH) == 0) {
              temp_score += 10;
              printf("*** CODONS exactly THE SAME --> '%.3s' and '%.3s'\n", sample_segment + sample_offset, candidate_segment + candidate_offset);
              continue;
          }

          // check if the codons specify the same amino acid (5 points) 
          if (strcmp(codon_names[get_codon_index(sample_segment + sample_offset)], codon_names[get_codon_index(candidate_segment + candidate_offset)]) == 0) {
              temp_score += 5;
              printf("*** CODONS the same AMINO ACID --> '%.3s' and '%.3s'\n", sample_segment + sample_offset, candidate_segment + candidate_offset);
              continue;
          }

          printf("No CODON matches found.\n");    
          
          for (int nucleotide = 0; nucleotide < CODON_LENGTH; ++nucleotide) {
              sample_offset = codon * CODON_LENGTH + nucleotide;
              candidate_offset = sample_offset + (iterations * CODON_LENGTH);

              // check if the nucleotides are equal
              printf("### Comparing NUCLEOTIDES --> '%.1s' and '%.1s'\n", sample_segment + sample_offset , candidate_segment + candidate_offset);

              if (*(sample_segment + sample_offset) == *(candidate_segment + candidate_offset)) {
                  temp_score += 2;
                  printf("*** NUCLEOTIDES exactly THE SAME --> '%.1s' and '%.1s'\n", sample_segment + sample_offset, candidate_segment + candidate_offset);
                  continue;
              }

              // check ff the nucleotides are a base pair
              if (is_base_pair(*(sample_segment + sample_offset), *(candidate_segment + candidate_offset))) {
                  temp_score += 1;
                  printf("*** NUCLEOTIDES are a BASE PAIR --> '%.1s' and '%.1s'\n", sample_segment + sample_offset, candidate_segment + candidate_offset);
                  continue;
              }

              printf("No NUCLEOTIDE matches found.\n");
          }
      }


      printf("\n# Calculated score of %d, comparing with %d\n", temp_score, score);

      if (temp_score > score) score = temp_score;

      printf("\n-- END OF ITERATION, SHIFTING... --\n");
  }
