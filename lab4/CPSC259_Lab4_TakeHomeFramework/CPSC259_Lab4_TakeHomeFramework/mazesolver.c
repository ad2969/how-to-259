...
void process()
{
  ...
  // generate_all_paths(...
  generate_all_paths(&paths, &paths_found, maze, dimension, 0, 0, "\0");

  /* Calculates and displays required data */
  strcpy(outputstring, "\0");
  construct_num_solutions_info(paths_found, outputstring);
...
void generate_all_paths( char*** pathsetref, int* numpathsref, maze_cell** maze, int dimension, int row, int column, char* path )
{
  ...
  /* Checks for base cases */
	if (row < 0 || row > dimension - 1 || column < 0 || column > dimension - 1
		|| maze[row][column].character == MAZE_WALL || maze[row][column].visited == VISITED) {
		return;
	}
  ...
  /* Otherwise deals with the recursive case.  Pushes the current coordinate onto the path
	  ...
	  ELSE the current location is marked as used and the search continues
		...
		// save if right boundary reached
...
int path_cost ( char* path_string )
{
  ...
  for (int i = 0; i < strlen(path_string); i++) {
	  strncpy(char_buffer, &path_string[i], 1);
	  sscanf(char_buffer, "%i", &int_buffer);
	  cost += int_buffer;
  }
...
void construct_num_solutions_info( int numpaths, char* outputbuffer )
{
	char int_buffer[BUFFER];

	strcat(outputbuffer, "Total number of solutions: ");

	sprintf(int_buffer, "%d", numpaths);
...
void construct_shortest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	...
	int shortest_path_index = -1;
	int path_length = 0;

	for (int i = 0; i < numpaths; i++) {
		path_length = strlen(pathset[i]);
		if (shortest_path_length == -1 || path_length < shortest_path_length) {
	...
	strcat(outputbuffer, pathset[shortest_path_index]);
	strcat(outputbuffer, "\n");
}
...
{
	int cheapest_path_cost = -1;
	int cheapest_cost_index = -1;
	int calculated_cost = 0;
	char int_buffer[BUFFER];

	for (int i = 0; i < numpaths; i++) {
		calculated_cost = path_cost(pathset[i]);
		if (cheapest_path_cost == -1 || calculated_cost < cheapest_path_cost) {
			cheapest_path_cost = calculated_cost;
			cheapest_cost_index = i;
		}
	}

	strcat(outputbuffer, "Cheapest path: ");
	strcat(outputbuffer, pathset[cheapest_cost_index]);
	strcat(outputbuffer, "\n");

	sprintf(int_buffer, "%d", cheapest_path_cost);
